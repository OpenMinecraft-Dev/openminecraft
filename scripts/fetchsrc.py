    ...
    while True:
        try:
            savefile(
                k,
                urlopen(
                    f"https://resources.download.minecraft.net/{v['hash'][:2]}/{v['hash']}"
                ).read(),
            )
        ...
        with BytesIO(urlopen(vermeta["downloads"]["client"]["url"]).read()) as bio:
            with zipfile.ZipFile(bio, "r") as zf:
                for n in zf.infolist():
                    if (
                        n.filename.startswith("assets/")
                        and not n.is_dir()
                        and needed(n.filename)
                    ):
                        print(f"Saving {n.filename}")
                        savefile(n.filename[7:], zf.open(n.filename).read())
        print("Fetching assets data")
        assets = json.loads(
            str(urlopen(vermeta["assetIndex"]["url"]).read(), encoding="utf-8")
        )
        thrs = []
