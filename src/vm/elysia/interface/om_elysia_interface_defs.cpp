...
static OMElysiaNativeHandle *interfaceAllocObject(OMElysiaJNIEnv *env, OMElysiaKlass *klass)
{
    ...
    execWithState(InsideVM, [&]() {
        hnd = env->internal->elysium->executor->recordLocalRef(env->internal->elysium->oopManager->allocateOop(klass));
    });
    return hnd;
};
...
static OMElysiaNativeHandle *interfaceNewStringUTF(OMElysiaJNIEnv *env, const char *string)
{
    ...
    OMElysiaNativeHandle *hnd;
    execWithState(InsideVM, [&]() {
        hnd = env->internal->elysium->executor->recordLocalRef(env->internal->elysium->oopManager->allocateString(ss));
    });
    return hnd;
}
