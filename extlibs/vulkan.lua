...
add_urls("https://github.com/KhronosGroup/Vulkan-Loader/archive/$(version).tar.gz", {
...
on_install("windows", "linux", "macosx", "cross", function(package)
	...
	if package:is_plat("linux") then
		import("package.tools.cmake").install(
			package,
			configs,
			{ packagedeps = { "libxrandr", "libxrender", "libxcb", "libxkbcommon" } }
		)
	else
...
on_install("macosx", "iphoneos", function(package)
	plat2 = package:is_plat("iphoneos") and (package:is_arch("x86_64") and "iossim" or "ios") or "macos"
		local plat = package:is_plat("iphoneos") and "iOS" or "macOS"
		io.replace("fetchDependencies", "-quiet", "")
		io.replace("Makefile", "-quiet", "")
        os.vrunv("./fetchDependencies", {"--" .. plat2})
        ...
        os.mv(path.join(moltenvk_rootdir, "MoltenVK.xcframework", plat2 .. "-*", "*.a"), package:installdir("lib"))
