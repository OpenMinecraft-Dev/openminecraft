...
class OMLogger
{
  public:
    ...
    template <typename... T> void debug(std::string msg, T... args)
    {
        debug(fmt::format(msg, args...));
    }
    void info(std::string msg);
    template <typename... T> void info(std::string msg, T... args)
    {
        info(fmt::format(msg, args...));
    }
    void warn(std::string msg);
    template <typename... T> void warn(std::string msg, T... args)
    {
        warn(fmt::format(msg, args...));
    }
    void error(std::string msg);
    template <typename... T> void error(std::string msg, T... args)
    {
        error(fmt::format(msg, args...));
    }
    void fatal(std::string msg);
    template <typename... T> void fatal(std::string msg, T... args)
    {
