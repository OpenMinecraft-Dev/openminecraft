#ifndef OM_UTIL_CRC_HPP
#define OM_UTIL_CRC_HPP

#include <array>
#include <cstdint>
namespace openminecraft::util
{
constexpr std::array<uint64_t, 256> crcTable = {
...
static auto calcCrc(uint64_t crc, void *dd, int length) -> uint64_t
{
    uint64_t c = crc;
    for (int n = 0; n < length; n++)
    {
        c = openminecraft::util::crcTable[(c ^ reinterpret_cast<uint8_t *>(dd)[n]) & 0xff] ^ (c >> 8);
    }
