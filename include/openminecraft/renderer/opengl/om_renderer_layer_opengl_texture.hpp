...
{
  public:
    OMRendererTextureOpenGL(uint64_t width, uint64_t height, uint64_t layers, uint64_t mipmap,
                            common::OMTextureType type, common::OMTextureArrangement arr, OMRendererOpenGL *renderer);
    ~OMRendererTextureOpenGL() override;

    void updateData(void *, uint64_t) override;
